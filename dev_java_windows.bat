...
REM -----------------------------------------------------------------------------
REM  Same behaviour as dev_java_unix.sh:
REM    1) Package with Maven (only when needed) and run the packaged JAR
REM    2) Run without compiling (java source-code launcher, no .class files)
REM    3) Compile (only if needed) then run from compiled classes
REM    4) Clean build artifacts (also restores the default configuration)
REM    5) Restore default configuration
REM    6) Exit
...
call :jar_state JAR_STATE
echo.
echo ========================================
echo     PlantUML GUI - Developer Launcher
echo ========================================
...
echo   jar     : %JAR_STATE%
echo ----------------------------------------
echo   1^) Package ^(if needed^) and run the JAR
echo   2^) Run without compiling ^(source mode^)
echo   3^) Compile ^(if needed^) and run
echo   4^) Clean build artifacts + reset config
echo   5^) Restore default configuration
echo   6^) Exit
echo ----------------------------------------
set "CHOICE="
...
if "%CHOICE%"=="1" ( call :run_packaged_jar        & call :pause_menu & goto :menu )
if "%CHOICE%"=="2" ( call :run_from_source          & call :pause_menu & goto :menu )
if "%CHOICE%"=="3" ( call :compile_and_run          & call :pause_menu & goto :menu )
if "%CHOICE%"=="4" ( call :clean_artifacts          & call :pause_menu & goto :menu )
if "%CHOICE%"=="5" ( call :restore_config_interactive & call :pause_menu & goto :menu )
if "%CHOICE%"=="6" ( echo [INFO]  Bye^! & exit /b 0 )
if "%CHOICE%"==""  ( echo [FAIL]  No option entered. Please type a number between 1 and 6. & goto :menu )
echo [FAIL]  Invalid option: '%CHOICE%'. Please type a number between 1 and 6.
goto :menu

REM =============================================================================
REM  Option 1 - Package with Maven when needed, then run the packaged JAR
REM =============================================================================
REM One `mvn package` produces both artifacts: the self-contained JAR in the
REM build dir and the distributable zip (JAR + user launchers) in the release dir.
:run_packaged_jar
call :is_jar_fresh JAR_FRESH
if "%JAR_FRESH%"=="1" (
    echo [ OK ]  Packaged JAR is up to date - skipping Maven.
) else (
    echo [INFO]  Packaged JAR is missing or outdated.
    call :build_jar
...
where mvn >nul 2>&1
if errorlevel 1 (
    echo [FAIL]  'mvn' was not found on PATH. Install Maven, or use option 3 ^(compile then run^).
    exit /b 1
)
echo [INFO]  Packaging with Maven ^(%JAR_FILE% and %ZIP_FILE%^)...
call mvn -B -q package
if errorlevel 1 (
    echo [FAIL]  Maven packaging failed.
    exit /b 1
)
if not exist "%JAR_FILE%" (
    echo [FAIL]  Maven finished but %JAR_FILE% was not produced.
    exit /b 1
)
echo [ OK ]  Packaged %JAR_FILE%
echo [ OK ]  Release zip %ZIP_FILE%
exit /b 0

REM Fresh = the JAR and the release zip exist and no source or resource file is
REM newer than the JAR. Windows has no 'find -newer', so the comparison is
REM delegated to PowerShell; without PowerShell the JAR is reported as outdated,
REM which just repackages.
:is_jar_fresh
set "%~1=0"
if not exist "%JAR_FILE%" exit /b 0
if not exist "%ZIP_FILE%" exit /b 0
where powershell >nul 2>&1
if errorlevel 1 exit /b 0
set "PS_FRESH=$r=(Get-Item '%JAR_FILE%').LastWriteTime; if (Get-ChildItem -Path '%SRC_DIR%','%RES_DIR%' -Recurse -File ^| Where-Object { $_.LastWriteTime -gt $r } ^| Select-Object -First 1) { 0 } else { 1 }"
for /f %%R in ('powershell -NoProfile -Command "!PS_FRESH!"') do set "%~1=%%R"
exit /b 0

REM =============================================================================
REM  Option 2 - Run directly from sources (no javac, no .class files)
REM =============================================================================
...
if %JAVA_MAJOR% EQU 0 (
    ...
    echo [FAIL]  Use option 3 ^(compile then run^) instead.
    exit /b 1
    ...
    if /i not "!ANSWER!"=="y" if /i not "!ANSWER!"=="yes" (
        echo [INFO]  Aborted. Use option 3 ^(compile then run^).
        exit /b 0
    )
)
...
REM =============================================================================
REM  Option 3 - Compile if needed, then run from compiled classes
REM =============================================================================
:compile_and_run
...
REM =============================================================================
REM  Option 4 - Clean build artifacts + restore the default configuration
REM =============================================================================
:clean_artifacts
set "FOUND="
...
if not defined FOUND if "%STRAY%"=="0" if not exist "%ZIP_FILE%" echo   - nothing, no build artifacts found
for %%D in (%FOUND%) do (
    set "N=0"
...
REM The release dir itself is source: it holds the user launchers, so only the zip goes.
...
REM =============================================================================
REM  Option 5 - Restore the default configuration only
REM =============================================================================
...
call :ini_get launcher.buildDir    "target"                           BUILD_DIR
call :ini_get launcher.releaseDir  "release"                          RELEASE_DIR
call :ini_get launcher.jarName     "plantuml-gui-java.jar"            JAR_NAME
call :ini_get launcher.zipName     "plantuml-gui-java.zip"            ZIP_NAME
call :ini_get launcher.mainClass   "com.diosaraiva.plantumlgui.Main"  MAIN_CLASS
call :ini_get launcher.cleanDirs   "bin out build target output"      CLEAN_DIRS
...
set "JAR_FILE=%BUILD_DIR%\%JAR_NAME%"
