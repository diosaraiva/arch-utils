...
# -----------------------------------------------------------------------------
#  Interactive menu:
#    1) Package with Maven (only when needed) and run the packaged JAR
#    2) Run without compiling (java source-code launcher, no .class files)
#    3) Compile (only if needed) then run from compiled classes
#    4) Clean build artifacts (also restores the default configuration)
...
load_settings() {
    ...
    BUILD_DIR="$(ini_get launcher.buildDir target)"
    RELEASE_DIR="$(ini_get launcher.releaseDir release)"
    JAR_NAME="$(ini_get launcher.jarName plantuml-gui-java.jar)"
    ZIP_NAME="$(ini_get launcher.zipName plantuml-gui-java.zip)"
    MAIN_CLASS="$(ini_get launcher.mainClass com.diosaraiva.plantumlgui.Main)"
    CLEAN_DIRS="$(ini_get launcher.cleanDirs 'bin out build target output')"
...
require_maven() {
    if ! command -v mvn >/dev/null 2>&1; then
        err "'mvn' was not found on PATH. Install Maven, or use option 3 (compile then run)."
        return 1
    fi
    return 0
}
...
# =============================================================================
# Option 1 - Package with Maven when needed, then run the packaged JAR
# =============================================================================
# One `mvn package` produces both artifacts: the self-contained JAR in the build
# dir and the distributable zip (JAR + user launchers) in the release dir.
# The JAR is stale when any source or resource is newer than it, using the same
# 'find -newer' comparison as the compiled-classes check.
is_jar_fresh() {
    [ -f "$JAR_FILE" ] || return 1
    [ -f "$ZIP_FILE" ] || return 1
    newer="$(find "$SRC_DIR" "$RES_DIR" -type f -newer "$JAR_FILE" 2>/dev/null | head -n 1)"
    [ -z "$newer" ]
}

build_jar() {
    require_maven || return 1
    info "Packaging with Maven ($JAR_FILE and $ZIP_FILE)..."
    mvn -B -q package
    status=$?
    if [ "$status" -ne 0 ] || [ ! -f "$JAR_FILE" ]; then
        err "Maven packaging failed (exit code $status)."
        return 1
    fi
    ok "Packaged $JAR_FILE"
    ok "Release zip $ZIP_FILE"
    return 0
}
...
run_packaged_jar() {
    ...
    if is_jar_fresh; then
        ok "Packaged JAR is up to date - skipping Maven."
    else
        info "Packaged JAR is missing or outdated."
        build_jar || return 1
    fi

    info "Running $JAR_FILE..."
    java -jar "$JAR_FILE"
    status=$?
...
# =============================================================================
# Option 2 - Run directly from sources (no javac, no .class files)
# =============================================================================
...
run_from_source() {
    ...
    if [ "$jv" -eq 0 ]; then
        ...
        err "Use option 3 (compile then run) instead."
        return 1
    fi
    if [ "$jv" -ne 0 ] && [ "$jv" -lt 22 ]; then
        ...
        case "$answer" in
            [yY]|[yY][eE][sS]) ;;
            *) info "Aborted. Use option 3 (compile then run)."; return 0 ;;
        esac
    fi
...
# =============================================================================
# Option 3 - Compile if needed, then run from compiled classes
# =============================================================================
# Freshness check:
...
# =============================================================================
# Option 4 - Clean build artifacts + restore the default configuration
# =============================================================================
clean_artifacts() {
    found=""
    ...
    if [ -z "$found" ] && [ "$stray_classes" -eq 0 ] && [ ! -f "$ZIP_FILE" ]; then
    ...
    # The release dir itself is source: it holds the user launchers, so only the zip goes.
    if [ -f "$ZIP_FILE" ]; then
        printf '  %s- %s%s\n' "$YELLOW" "$ZIP_FILE" "$RESET"
    fi
    ...
    if [ -f "$ZIP_FILE" ] && rm -f -- "$ZIP_FILE"; then
...
# =============================================================================
# Option 5 - Restore the default configuration only
# =============================================================================
...
show_menu() {
    printf '\n%s%s========================================%s\n' "$BOLD" "$BLUE" "$RESET"
    printf '%s%s    PlantUML GUI - Developer Launcher   %s\n'   "$BOLD" "$BLUE" "$RESET"
    printf '%s%s========================================%s\n'   "$BOLD" "$BLUE" "$RESET"
    printf '  project : %s\n' "$PROJECT_DIR"
    printf '  config  : %s\n' "$CONFIG_FILE"
    ...
    if is_jar_fresh; then
        printf '  jar     : %sup to date%s\n' "$GREEN" "$RESET"
    elif [ -f "$JAR_FILE" ]; then
        printf '  jar     : %soutdated%s\n' "$YELLOW" "$RESET"
    else
        printf '  jar     : %snot packaged%s\n' "$RED" "$RESET"
    fi
    printf '%s----------------------------------------%s\n' "$BLUE" "$RESET"
    printf '  %s1)%s Package (if needed) and run the JAR\n' "$BOLD" "$RESET"
    printf '  %s2)%s Run without compiling (source mode)\n'  "$BOLD" "$RESET"
    printf '  %s3)%s Compile (if needed) and run\n'          "$BOLD" "$RESET"
    printf '  %s4)%s Clean build artifacts + reset config\n' "$BOLD" "$RESET"
    printf '  %s5)%s Restore default configuration\n'        "$BOLD" "$RESET"
    printf '  %s6)%s Exit\n'                                 "$BOLD" "$RESET"
    printf '%s----------------------------------------%s\n' "$BLUE" "$RESET"
}
...
main() {
    ...
    while true; do
        show_menu
        printf '%sChoose an option [1-6]: %s' "$BOLD" "$RESET"
        ...
        case "$choice" in
            1) run_packaged_jar; pause ;;
            2) run_from_source; pause ;;
            3) compile_and_run; pause ;;
            4) clean_artifacts; pause ;;
            5) restore_config_interactive; pause ;;
            6) info "Bye!"; exit 0 ;;
            "") err "No option entered. Please type a number between 1 and 6." ;;
            *) err "Invalid option: '$choice'. Please type a number between 1 and 6." ;;
        esac
    done
}
